...
public class AliyunVideo {
    //账号AK信息请填写(必选)
    private static String access_key_id = "mbIod2znoXByW06V";
    //账号AK信息请填写(必选)
    private static String access_key_secret = "EDfIOxYvyzdqW4yozpNadTYhRcoZmS";
    ...
    private final static Logger LOG = Logger.getLogger(AliyunVideo.class.getName());

    public static void SendRequest(Map<String,String> map)throws IOException{
        //生成私有参数，不同API需要修改
        Map<String, String> privateParams = generatePrivateParamters(map);
        //生成公共参数，不需要修改
     ...
     * @return
     */
    private static Map<String, String> generatePrivateParamters(Map<String,String> map) {
        // 接口私有参数列表, 不同API请替换相应参数
        Map<String, String> privateParams = new HashMap<>();
        // 视频ID
        if("DeleteVideo".equals(map.get("Action").toString())) {
            privateParams.put("VideoIds", map.get("VideoId").toString());
        }else{
            privateParams.put("VideoId", map.get("VideoId").toString());
        }

        // API名称
        privateParams.put("Action", map.get("Action").toString());
        if (map.containsKey("Title")) {
            // 视频标题
            privateParams.put("Title", map.get("Title"));
        }
        if (map.containsKey("Description")) {
            // 视频描述
            privateParams.put("Description", map.get("Description"));
        }
        if (map.containsKey("CoverURL")) {
            // 视频封面地址
            privateParams.put("CoverURL", map.get("CoverURL"));
        }
    ...
    */
    ...
    public static DefaultAcsClient getClient(){
        return  new DefaultAcsClient(DefaultProfile.getProfile("cn-shanghai", access_key_id, access_key_secret));
    }
}